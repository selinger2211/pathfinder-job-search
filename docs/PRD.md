...
|---------|------|---------|
| v3.20.4 | 2026-03-13 | **Resume Builder spacing polish + full QA pass** — DOCX role spacing, skills bar indent, preview CSS alignment. QA fixes: added missing Resume Builder nav bar, fixed Pipeline Sync Hub link, cache-bust params, safeJsonParse() hardening across all modules. |
| v3.18.0 | 2026-03-13 | **FINAL PLANNED FEATURE: Calendar Phase 4 Intelligence & Feedback Loops (#53)** — Completion of all 53 planned features across Pathfinder. Calendar Module gains full intelligence and analytics: (1) Interviewer Reputation Scoring — encounter count, average rating, question types asked, outcome correlation (screens to next round rate). (2) Outcome Pattern Analysis — analysis across day/time/interview type/company tier/prep level to identify patterns. (3) Success Rate Tracking — overall rate, by stage (screen→offer), by tier, trend over time with sparklines. (4) Predictive Insights — 3-5 actionable suggestions based on historical patterns (e.g., "Better outcomes with Manager interviews on Tuesdays," "Avoid evening slots with this company"). (5) Full Intelligence Tab — new analytics view with stats cards (upcoming/completed interviews, success rate, next steps), charts (conversion funnel, weekly volume, interviewer performance), and pattern drill-downs. THIS IS THE FINAL SCHEDULED FEATURE. All 53 planned features now complete. |
| v3.17.0 | 2026-03-13 | **Tier 3 Features: 5 features across Pipeline, Job Feed, Calendar, Research Brief** — Major release completing auto-enrichment, email/career page monitoring, post-interview automation, and research brief intelligence. (1) Pipeline (#39): Auto-Enrichment on Company Entry with DuckDuckGo Instant Answer API + Clearbit Logo + Google Favicon + website meta tags. Enrichment panel, re-enrich button, batch "Enrich All" with progress. (2) Job Feed (#40, #41): Gmail Integration with token-based OAuth UI, email scanner (Gmail API search for job emails), parsed emails as feed items with "Gmail" badge, auto-scan with cooldown. Career Page Monitoring with tracked company URLs, auto-detect ATS (Greenhouse/Lever/Ashby/generic), career page crawler, new job detection vs cache, "Check All" button, daily auto-check. (3) Calendar (#42): Post-Interview Triggers with rejection signal detection (silence + email patterns), next round detection (event title patterns), offer detection (calendar + email), stage auto-advance with transition reason. (4) Research Brief (#43): Deep Integrations Phase 3 with interviewer profiles section (section 14) from connections data, cross-module invalidation hub (7 signal sources), brief completeness score (0-100 with progress bar), invalidation dashboard. |
| v3.16.0 | 2026-03-13 | **Smart Caching Phase 2 + Phase 4 Polish + MCP Features: 6 features across Research Brief, Job Feed, Resume Builder, MCP Server** — Major release completing intelligent caching system and MCP server integration. (1) Research Brief (#51, #52): Smart Caching Phase 2 with cross-module invalidation signals (pipeline, company, comp, debrief), section-level staleness tracking with source hashing, smart regeneration of only stale sections. Phase 4 Polish with enhanced citation popovers (URL, date, trust level, refresh button), improved progress bar showing section names and ETAs, keyboard shortcuts (G/E/R/Esc/arrows), sticky section navigation sidebar (200px left). (2) Research Brief (#28): MCP-Based Artifact Persistence with `pf_save_brief`, `pf_get_brief`, `pf_list_briefs`, `pf_compare_briefs` tools. SQLite research_briefs table, "Save to Server" button, "Brief History" dropdown in UI. (3) Job Feed (#35): MCP Tool Integration with `pf_search_feed` and `pf_get_role` tools, MCP status indicator (green/gray dot) in feed header. (4) MCP Server (#36): New `pf_generate_brief_section` tool for server-side brief section generation via Claude API. (5) Resume Builder (#49): MCP-Powered Generation with `pf_generate_bullets` and `pf_export_resume` tools, MCP Generate toggle, "Export via MCP" button in UI. |
...
| v3.13.0 | 2026-03-13 | **Cross-Module Wiring: 12 Tier 2 features across 10 modules** — Major release completing second-order integrations. (1) Dashboard: Feed Review Section with top 5 unreviewed items, Accept/Snooze/Dismiss buttons. (2) Outreach: Thank You (Interview) message type with debrief integration, InMail with preview mode and shared connections. (3) Debrief: Next Steps Panel with "Refresh Research Brief" + "Draft Thank-You" action cards, writes `pf_brief_invalidation` signal, updates `role.lastDebriefAt`. (4) Research Brief: Auto Cache Invalidation checks `pf_brief_invalidation` on load with stale section banners, "Regenerate Stale Sections" button. Export Formats include HTML and Markdown alongside PDF with dropdown menu. (5) Calendar: Confidence Scoring Algorithm (0-100, auto-match 70+, suggestions 40-69), JD-calibrated questions cached per role+type. Interviewer Linking matches attendees to connections/LinkedIn with seniority badges. (6) Job Feed: Automatic Dedup vs Pipeline with exact+fuzzy matching, "In Pipeline" and "Possible Duplicate" badges. (7) Pipeline: Interview & Offer Substages (Phone Screen, Technical, Onsite, Final Round, Team Match for interviews; Verbal, Written, Negotiating, Accepted, Declined for offers). (8) Mock Interview: Intelligence & Calibration with JD-calibrated questions, post-session summary with strengths/improvements, analytics tab (sessions/week, type coverage, streak, top areas). (9) Comp Intelligence: Positioning Intelligence (IC vs Director comparison, cross-company table, percentile position) and Advanced Analytics (comp by funding stage, comp by tier, stats cards). (10) Resume Builder: Feedback Loops with keyword gap analysis, Claude-generated bullet suggestions, "Add to Bullet Bank" buttons. |
| v3.12.0 | 2026-03-13 | **Tier 1 Quick Wins: 14 features across Dashboard (funnel, time-in-stage, sparse nudge, dedup, logging, prefs), Research Brief (citations, status badges, progress bar, keyboard shortcut), Pipeline (stage reasons, company search), Feed (run logging), Calendar (nudge timing)** — Completion of major feature release. (1) Dashboard: Conversion Funnel Card shows stage-to-stage conversion rates (after 10+ closed roles). (2) Average Time-in-Stage Card displays median days per stage as sortable table. (3) Company Profile Sparse Nudge (Rule 13) fires when company <50% enriched. (4) Nudge Deduplication UI groups by roleId, shows highest-priority, "+" expander for additional nudges. (5) Nudge Logging captures all nudges to `pf_nudge_log` localStorage (generated/dismissed/acted for analytics). (6) Per-Rule Nudge Preferences sidebar toggles for all 13 nudge rules, user preferences in `pf_nudge_prefs` localStorage. (7) Research Brief Citation Markers: clickable [n] popovers showing source/date/trust. (8) Section Status Badges: Fresh/Cached/Stale/Error/Generating pills per section. (9) Generation Progress Bar: real-time "Generating section 3/13..." indicator. (10) Keyboard Shortcut G: press G to trigger generate. (11) Pipeline Stage Transition Reason Field: optional reason chips when changing stage, saved to stageHistory. (12) Company Web Search Suggestions: DuckDuckGo instant answer API on company name input. (13) Feed Run Logging: `pf_feed_run_log` localStorage tracks run stats in header. (14) Calendar Specific Nudge Timing: 72h/48h/morning-of/post-interview nudge cards with countdowns. |
| v3.11.0 | 2026-03-13 | **Bug fixes + UX improvements across 4 modules** — 6 backlog items addressed: (1) Smart outreach nudges: Dashboard nudge engine now reads comms log context for follow-up suggestions, surfaces mutual connections from `pf_connections` + `pf_linkedin_network`, shows last email subject and snippet, offers "Draft Follow-up" and "View Connections" action buttons. (2) Pipeline default sort by score: Table view defaults to score descending, new Score column with color-coded values, kanban cards sort within columns by score. (3) Company visibility: Company names clickable → Research Brief across Feed/Pipeline, hover tooltips with mission statement, brief descriptions on cards. (4) Full JD sidebar panel: Slide-from-right detail panel (480px) shows full JD text, source badge, confidence indicator — same pattern as Pipeline's detail panel. (5) Comp estimate labeling: "Posted Base" vs "Est. Total Comp" with info tooltip explaining methodology. (6) Company descriptions extended to Research Brief card. |
| v3.10.0 | 2026-03-12 | **Free JD enrichment engine + nav reorder + logo fixes** — Replaced entire Apify-based enrichment with zero-cost direct engine: 3-strategy approach using CORS proxies for LinkedIn JD extraction, public ATS APIs (Greenhouse/Lever/Ashby), and DuckDuckGo job board search fallback. Auto-enrichment re-enabled on page load (1.5s delay between fetches). Research Brief JD company mismatch guard (validates JD mentions company name, system prompt declares `<company>` tag as authoritative). Feed logo fix: `renderCardLogo()` uses `getCompanyDomain()` instead of `item.domain` (which is industry category). LinkedIn favicon fix: `getCompanyDomain()` skips LinkedIn job URLs, falls through to name-based domain. Nav tabs reordered chronologically across all 11 modules: Dashboard → Feed → Pipeline → Research → Resume → Outreach → Comp → Mock → Debrief → Calendar → Sync Hub. Sidebar Apify UI replaced with simple "JD Enrichment" info panel (no API keys needed). |
